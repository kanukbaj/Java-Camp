...
public class JobAdvertisementManager implements JobAdvertisementService {
	...
	public Result addNew(JobAdvertisementForAddDto jobAdvertisement) {
		...
		//Aslında burası authentication dn gelecek fakat şu an öğrenemedik, o yüzden el ile istiyoruz :)
	...
	public Result changeStatus(int advertisementId, int employerId) {
		...
		if(jobAdvertisementToUpdate == null) return new ErrorResult("Bu kriterlere uyan bir iş ilanı bulunamadı. Böyle bir iş ilanı yok veya bu iş ilanı bu şirkete ait değil.");
	...
	private Result isJobPositionValid(int id) {
		if(id<=0) return new ErrorResult("iş pozisyonu doğru girilmedi.");
